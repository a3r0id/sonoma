...
# RETURNS HEADER OBJECT IF EXISTING: {"name": "", "value": ""} or None
def getHeader(headers, headerName, toLower=False):

    if toLower:
        headerName = headerName.lower()

    for header in headers:
        thisHeaderName = header[0]

        if toLower:
            thisHeaderName = thisHeaderName.lower()
...
class httpServer(object):
    ...
    def editConfig(self, **editParams):
        ...
        ```python
        server.editConfig(self, str httpVersion=None, str serverName=None, str defaultOrigin=None, int maxHeaderNameLength=None, int maxHeaderValueLength=None, int maxRecv=None, str or bytes defaultResponse=None)
        ```
        # Config:
        ...
        - maxHeaderNameLength: Maximum header key length accepted before responding with `413 - Entity Too Large` error.
            ...
            while True:    
                ...
                for unit in requestHeaders_buffer:
                    if unit != "" and ": " in unit:
                        ...
                        if len(key) > defaults['maxHeaderNameLength'] or len(value) > defaults['maxHeaderValueLength']:
